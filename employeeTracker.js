...
function updateEmployee() {
  var query =
    "SELECT e.first_name, e.last_name, r.employeerole, e.manager_id, e.id FROM employee_db.employee as e LEFT JOIN employeerole as r on e.role_id = r.id";
  var employees = [];
  connection.query(query, function (err, res) {
    if (err) throw err;
    for (var i = 0; i < res.length; i++) {
    ...
    inquirer
      ...
      .then(function (res) {
        ...
        connection.query(
          query,
          [res["update role"], res["which employee"]],
          function (err, res) {
            if (err) throw err;
